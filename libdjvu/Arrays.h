...
/** @name Arrays.h
    ...
    Class \Ref{TArrayDj} implements an array of objects of trivial types
    ...
    \Ref{TArrayDj} with non-trivial classes, it should not be done.
    ...
    sorting. Both \Ref{TArrayDj} (dynamic array for trivial types) and
    ...
    At this point \Ref{DArray} and \Ref{TArrayDj} should only be used when
...
/** Dynamic array base class.
    This is an auxiliary base class for \Ref{DArray} and \Ref{TArrayDj}
...
/** Dynamic array template base class.
    This is an auxiliary template base class for \Ref{DArray} and \Ref{TArrayDj}
...
/** Dynamic array for simple types.  
    Template class #TArrayDj<TYPE># implements an array of
    elements of {\em simple} type #TYPE#. {\em Simple} means that the type
    may be #char#, #int#, #float# etc. The limitation is imposed by the
    way in which the #TArrayDj# is working with its elements: it's not trying
...
template <class TYPE>
class TArrayDj : public ArrayBaseT<TYPE> {
public:
...
template <class TYPE> void
TArrayDj<TYPE>::destroy(void * data, int lo, int hi)
{
}

template <class TYPE> void
TArrayDj<TYPE>::init1(void * data, int lo, int hi)
{
}

template <class TYPE> void
TArrayDj<TYPE>::init2(void * data, int lo, int hi,
		    const void * src, int src_lo, int src_hi)
{
   if (data && src)
   {
...
// inline removed
template <class TYPE> void
TArrayDj<TYPE>::insert(void * data, int els, int where,
		     const void * what, int howmany)
{
...
template <class TYPE> 
TArrayDj<TYPE>::TArrayDj(int lo, int hi)
{
   this->assign(new ArrayRep(sizeof(TYPE), destroy, init1,
