...
#define ASSERT_ENCB(expr) do{if(!(expr))G_THROW("assertion ("#expr") failed");}while(0)
...
{
  ASSERT_ENCB(size>0 && size<0x1000000);
  rank[size] = -1;
}
  ...
  while (--sp>=0)
    {
      ...
      if (hi-lo<RANKSORT_THRESH)
      ...
      else
        {
          ...
          h1 = hi - (h1-h);
          // -- process segments
	  ASSERT_ENCB(sp+2<QUICKSORT_STACK);
  ...
  while (--sp>=0)
    {
      ...
      if (depth >= PRESORT_DEPTH)
        ...
        else
        {
          ...
          h1 = hi - (h1-h);
          // -- process segments
	  ASSERT_ENCB(sp+3<QUICKSORT_STACK);
          // ----- middle segment (=?) [l1, h1]
          l = l1; h = h1;
...
_BSort::run(int &markerpos)
{
  ...
  ASSERT_ENCB(markerpos>=0 && markerpos<size);
#ifdef BSORT_TIMER
  long end = GOS::ticks();
...
BSByteStream::Encode::flush()
{
  if (bptr>0)
  {
    ASSERT_ENCB(bptr<(int)blocksize);
