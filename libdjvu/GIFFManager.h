...
protected:
   ...
   GIFFChunk(const GUTF8String &name, const TArrayDj<char> & data);
public:
      /// Default creator.
   ...
   static GP<GIFFChunk> create(const GUTF8String &name, const TArrayDj<char> & data)
   ...
   TArrayDj<char>	get_data(void) const;
...
inline
GIFFChunk::GIFFChunk(const GUTF8String &name, const TArrayDj<char> & data_in) :
      data(data_in)
{
   set_name(name);
}
...
public:
   ...
   void		add_chunk(GUTF8String parent_name, const GP<GIFFChunk> & chunk, int pos=-1);
	  ...
	  this case is either #-1# (append) or is extracted from between
          ...
          \end{verbatim} */
   void		add_chunk(GUTF8String name, const TArrayDj<char> & data);
